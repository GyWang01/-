...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/MutualVoltage_BarChart.png}
...
在完成基础性能测试后，本文进一步搭建了道岔仿损检测实验平台，对所研制便携式 EMT 系统在实际检测任务中的应用能力进行验证。实验对象采用含机加工开口缺陷的铁磁性试块，用以等效模拟道岔尖轨、基本轨等部位常见的剥离掉块、压溃掉块及局部缺口类表面或近表面伤损。需要指出的是，本文实验等效对象主要为开口型、材料缺失型伤损，对闭合疲劳裂纹、深部裂纹及复杂分层裂纹等非开口型伤损的检测能力仍需后续结合真实道岔试件和更多伤损类型进一步验证。

测试过程中，系统采集阵列线圈在不同激励条件下的复数响应，并分别采用传统 EMT 成像方法和 SCF-DeepONet 模型进行结果分析。
...
% \begin{figure}[!htb]
%     \centering
%     \includegraphics[width=0.82\textwidth]{figures/experiment.jpg}
%     \figcaption{伤损检测实验平台实物图}{Physical photo of the defect detection platform}
%     \label{fig:experiment_platform}
% \end{figure}
...
\begin{figure}[!htb]
    ...
    \begin{minipage}[c]{0.64\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/31.jpg}\\[-2pt]
        {\zihao{5}(a) 伤损检测平台正照 \\ {\zihao{5}(a) Front view of the damage detection platform}}
    \end{minipage}
    ...
    \begin{minipage}[c]{0.64\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/32.jpg}\\[-2pt]
        {\zihao{5}(b) 伤损检测平台背照\\ {\zihao{5}(b) Back view of the damage detection platform}}
    \end{minipage}
    \figcaption{伤损检测平台实物图}{Physical photo of the damage detection platform}
    \label{fig:experiment_platform}
\end{figure}
...
\begin{figure}[!htb]
    ...
    \begin{minipage}[t]{0.90\textwidth}
        \centering
        \includegraphics[width=0.82\textwidth]{figures/bulk.jpg}\\[-2pt]
